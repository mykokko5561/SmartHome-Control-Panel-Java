...
public class SmartHomeGUI extends JFrame {
    ...
    public SmartHomeGUI() {
        ...
        // --- SISTEM AÇILIŞINDA VERİTABANINDAN (DOSYADAN) DURUMLARI YÜKLE ---
        loadSystemStateFromDatabase();

        // --- SISTEM KAPANIRKEN SON DURUMU VERİTABANINA (DOSYAYA) KAYDET ---
    ...
    // --- DATABASE INTEGRATION (FILE I/O & RAM CACHE) ---
    private void saveSystemStateToDatabase() {
        ...
        // Verileri RAM'den fiziksel TXT dosyasına yaz:
        db.saveCacheToFile();
    }
    ...
    private void loadUserSession(String username) {
        ...
        if (state == null) {
            // Eğer uygulama yeni açıldıysa, sıfırdan RAM Session oluştur ve veritabanından yüklenen evin o anki fiziksel durumunu miras al!
            saveCurrentSession();
            return; 
        }
        
        resetAllDevices(); // Sadece eski bir oturum kaydı RAM'de bulunuyorsa sıfırla ve onun kayıtlarını yükle
